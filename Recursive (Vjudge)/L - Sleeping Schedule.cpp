...
int spell(int i, int curr_time) {
    if (i >= n) return 0;

    if (~dp[i][curr_time])
        return dp[i][curr_time];

    int ch1 = spell(i + 1, (curr_time + arr[i]) % h) + valid((curr_time + arr[i]) % h);
    int ch2 = spell(i + 1, (curr_time + arr[i] - 1) % h) + valid((curr_time + arr[i] - 1 + h) % h);

    return dp[i][curr_time] = max(ch1, ch2);
}
